...
Samples2 <- Samples %>% 
  ...
  mutate(Event = (cumsum(LowVelocity) - lag(cumsum(LowVelocity), n = 20))/20) %>%
  
  ## the next lines are very hacky
  ## if average is > .9, count as fixation
  mutate(FixationNum = ifelse(Event >= 0.90, 1, 0),
...
ggplot(subset(Samples2, ParticipantID == '2019-2019-5_1_f2' & Trial == 8 & !is.na(Event)),
...
Saccades <- Samples2 %>% 
  ...
  summarize(
    ...
    AmplitudeY = max(PositionX)-min(PositionY),
